...
#+begin_src emacs-lisp
  ...
  ;; add a new use-package keyword that will check
  ;; if there is a program installed on the system
  ;; and if not print an error that the system package is not installed
  #+end_src
...
*** Input method
I making a custom input method for written language.
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'completion-category-overrides
               '(eglot-capf (styles basic partial-completion substring flex)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    (setq corfu-sort-override-function corfu-sort-function)
...
#+begin_src emacs-lisp
  ...
  (load-theme 'misterioso t)
  (set-face-attribute 'region nil :background "#036767")
#+end_src
...
show what on what region undo works
...
#+begin_src emacs-lisp
  (use-package hl-line
    ...
    (set-face-background 'hl-line "#323c47"))
#+end_src
...
#+begin_src emacs-lisp
  (setenv "JAVA_HOME" "/usr/lib/jvm/java-latest-openjdk")
#+end_src
...
#+begin_src emacs-lisp
  (use-package dape
    ...
    ;; we disable repeat mode for dape commands because of meow
...
#+begin_src emacs-lisp
  (use-package treesit
    :ensure nil
    :config
    (setopt treesit-enabled-modes '()) ;; add to list to enable ts modes
...
#+begin_src emacs-lisp
  (use-package apheleia
    :defer t
    :config
    ;; setup goimports to work only on the current file
    ;; this way the formating is much faster
    (setf (alist-get 'goimports apheleia-formatters)
          '("goimports" "-srcdir" filepath))

    ;; use `goimpors' to format instad of `gofmt'
    ;; `gofmt' doesn't sort and format imports
  ...
  (use-package sly
    ...
    (setq sly-lisp-implementations
        `((sbcl    ("sbcl" "--dynamic-space-size" "2000"))  ;; default. Adapt if needed.
          (ciel-sbcl  ("sbcl"
                       "--core"  ,(expand-file-name "~/quicklisp/local-projects/CIEL/ciel-core")
...
#+begin_src emacs-lisp
  (use-package typescript-mode
    :hook ((typescript-mode-hook js-mode-hook) . eglot-ensure)
    :config
...
#+begin_src emacs-lisp
  (use-package scala-ts-mode
    :hook (scala-ts-mode-hook . eglot-ensure)
    :interpreter ("scala" . scala-ts-mode)
    :config
    (add-hook 'scala-ts-mode-hook
                (lambda ()
                  (setq-local compile-command "scala run ")))

*** Typst
#+begin_src emacs-lisp
  (use-package typst-ts-mode
    :vc (:url "https://codeberg.org/meow_king/typst-ts-mode.git")
    :hook (typst-ts-mode-hook . display-line-numbers-mode)
    :custom
...
#+begin_src emacs-lisp
  ...
  (defun kr-insert-enter-toggle ()
    (setq tab-always-indent 'complete))
  (add-hook 'meow-insert-enter-hook 'kr-insert-enter-toggle)

  (defun kr-insert-exit-toggle ()
    (setq tab-always-indent t))
  (add-hook 'meow-insert-exit-hook 'kr-insert-exit-toggle)
