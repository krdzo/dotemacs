...
* Keybindings
This section contains personal keybindings that are grouped in keymaps.
Each keymap is a logical grouping of keybindings for some purpose (buffers, windows, etc.) and then that keymap is bound to a prefix key so that there is only one place where we need to change the prefix key so that it works globaly.
After this ~meow~ can bind to the prefix key. This help in a way so that I don't need to configure meow separately.
** Buffer
#+begin_src emacs-lisp
...
Big packages like org-mode will get their own headings.
** Measuring start-up
